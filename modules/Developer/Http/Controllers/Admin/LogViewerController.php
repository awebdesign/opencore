...
class LogViewerController extends Controller
{
    ...
    /** @var string */
    protected $showRoute = 'developer::admin.logs.show';
     ...
     */
    public function __construct(LogViewerContract $logViewer, OcCore $occore)
    {
    ...
    public function show($date, Request $request)
    {
        ...
        $entries = $log->entries($level)->paginate($this->perPage)->appends([$this->occore->getTokenKey() => $this->occore->getToken()]);;
    ...
    public function showByLevel($date, $level, Request $request)
    {
        ...
        $query   = $request->get('query');
        $levels  = $this->logViewer->levelsNames();
        $entries = $this->logViewer->entries($date, $level)->paginate($this->perPage)->appends([$this->occore->getTokenKey() => $this->occore->getToken()]);
    ...
    public function search($date, $level = 'all', Request $request)
    {
        ...
        })->paginate($this->perPage)->appends([$this->occore->getTokenKey() => $this->occore->getToken()]);;
    ...
    protected function view($view, $data = [], $mergeData = [])
    {
        ...
        return view()->make("developer::admin.log-viewer.{$view}", $data, $mergeData);
    }
