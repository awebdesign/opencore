    ...
    <table class="uk-table uk-table-responsive" cellpadding="0" cellspacing="0" class="mb1">
        ...
        <tbody>
            @forelse($tasks as $task)
                <tr is="task-row"
                    :data-task="{{$task}}"
                    showHref="{{route('admin::core.tasks.view', $task)}}"
                    executeHref="{{route('admin::core.tasks.execute', $task)}}">
                </tr>
            @empty
                <tr>
...
@section('main-panel-footer')
    <div class="uk-flex uk-flex-between">
        <span>
            <a class="uk-icon-button uk-button-primary uk-hidden@m" uk-icon="icon: plus" href="{{route('admin::core.tasks.create')}}"></a>
            <a class="uk-button uk-button-primary uk-button-small uk-visible@m" href="{{route('admin::core.tasks.create')}}">New Task</a>
        </span>

        <span>
            <import-button url="{{route('admin::core.tasks.import')}}"></import-button>
            <a class="uk-icon-button uk-button-primary uk-hidden@m" uk-icon="icon: cloud-download"  href="{{route('admin::core.tasks.export')}}"></a>
            <a class="uk-button uk-button-primary uk-button-small uk-visible@m" href="{{route('admin::core.tasks.export')}}">Export</a>
        </span>
    </div>
