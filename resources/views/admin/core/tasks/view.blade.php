...
@section('title')
    <div class="uk-flex uk-flex-between uk-flex-middle">
        ...
        <status-button :data-task="{{ $task }}" :data-exists="{{ $task->exists ? 'true' : 'false' }}" activate-url="{{route('admin::core.tasks.activate')}}" deactivate-url="{{route('admin::core.tasks.deactivate', $task)}}"></status-button>
    </div>
@stop
...
@section('main-panel-footer')
    <div class="uk-flex uk-flex-between uk-flex-middle">
        <span>
            <a href="{{ route('admin::core.tasks.edit', $task) }}" class="uk-button uk-button-primary uk-button-small">Edit</a>
            <form class="uk-display-inline" action="{{route('admin::core.tasks.delete', $task)}}" method="post">
                {{ csrf_field() }}
                {{ method_field('delete') }}
                <button type="submit" class="uk-button uk-button-danger uk-button-small">Delete</button>
            </form>
        </span>
        <execute-button :data-task="{{ $task }}" url="{{route('admin::core.tasks.execute', $task)}}" button-class="uk-button-small uk-button-primary"></execute-button>
    </div>
@stop
